...
Proma Agent 模式，通用 Agent 能力，支持 Cladue 全系列、Minimax M2.1、Kimi K2.5、智谱 GLM 等模型，支持第三方模型供应商。优雅、简洁、丝滑、确信的流式输出。
...
Proma 全协议大模型供应商支持，支持国内外所有模型供应商，通过 Base URL + API KEY 配置。
...
进入 **设置 > 模型供应商**，点击 **添加模型供应商**，选择供应商并输入 API Key。Proma 会自动填充正确的 API 地址。点击 **测试连接** 验证，然后 **获取模型** 加载可用模型列表。

### Agent 模式（仅限 Anthropic）

Agent 模式需要一个 **Anthropic** 模型供应商。添加后，进入 **设置 > Agent** 选择你的 Anthropic 模型供应商和模型（推荐 Claude Sonnet 4 / Opus 4）。底层使用 [Claude Agent SDK](https://github.com/anthropics/claude-agent-sdk)。
