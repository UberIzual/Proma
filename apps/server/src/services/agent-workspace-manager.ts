 ...
 *
 * 基于 Electron 版本改造，使用 Bun 异步 API。
 */
 ...
 * 读取工作区索引文件（异步）
 */
async function readIndex(): Promise<AgentWorkspacesIndex> {
  ...
  try {
    ...
    return (await file.json()) as AgentWorkspacesIndex
  } catch (error) {
 ...
 * 写入工作区索引文件（异步）
 */
async function writeIndex(index: AgentWorkspacesIndex): Promise<void> {
  const indexPath = getAgentWorkspacesIndexPath()
 ...
 * 按 ID 获取单个工作区
 */
export async function getAgentWorkspace(id: string): Promise<AgentWorkspace | undefined> {
  const index = await readIndex()
 ...
 * 按 slug 获取单个工作区
 */
export async function getAgentWorkspaceBySlug(slug: string): Promise<AgentWorkspace | undefined> {
  const index = await readIndex()
 ...
 * 返回默认工作区的 ID。
 */
export async function ensureDefaultWorkspace(): Promise<AgentWorkspace> {
  const index = await readIndex()
 ...
 * 读取工作区 MCP 配置（异步）
 */
export async function getWorkspaceMcpConfig(workspaceSlug: string): Promise<WorkspaceMcpConfig> {
  ...
  try {
    ...
    const parsed = (await file.json()) as Partial<WorkspaceMcpConfig>
 ...
 * 保存工作区 MCP 配置（异步）
 */
export async function saveWorkspaceMcpConfig(workspaceSlug: string, config: WorkspaceMcpConfig): Promise<void> {
  const mcpPath = getWorkspaceMcpPath(workspaceSlug)
 ...
 */
export async function getWorkspaceCapabilities(workspaceSlug: string): Promise<WorkspaceCapabilities> {
