...
/**
 * 构建静态 system prompt 追加内容（异步）
 *
 * 拼接 Agent 角色定义、用户信息、工作区结构说明和交互规范。
 * 内容保持稳定以利用 Anthropic prompt caching。
 */
export async function buildSystemPromptAppend(ctx: SystemPromptContext): Promise<string> {
...
export async function buildDynamicContext(ctx: DynamicContext): Promise<string> {
  ...
  if (ctx.workspaceSlug) {
    ...
    // MCP 服务器列表
    const mcpConfig = await getWorkspaceMcpConfig(ctx.workspaceSlug)
