...
import { Bot, CornerDownLeft, Square, Settings, Paperclip, AlertCircle, X, FolderOpen } from 'lucide-react'
...
import type { AgentSendInput, AgentStreamEvent, AgentMessage, AgentPendingFile, ModelOption } from '@proma/shared'
...
export function AgentView(): React.ReactElement {
  ...
  React.useEffect(() => {
    ...
    const updateState = (
      ...
      setStreamingStates((prev) => {
        const current = prev.get(sessionId) ?? { running: true, content: '', reasoning: '', toolActivities: [], model: undefined }
  ...
  const handleSend = React.useCallback(async (): Promise<void> => {
    ...
    if ((!text && pendingFiles.length === 0) || !currentSessionId || !agentChannelId || streaming) return
  ...
  }, [inputContent, pendingFiles, currentSessionId, agentChannelId, agentModelId, currentWorkspaceId, workspaces, streaming, setStreamingStates, setCurrentMessages, setPendingFiles, setAgentStreamErrors])
