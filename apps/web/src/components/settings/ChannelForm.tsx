...
export function ChannelForm({ channel, onSaved, onCancel }: ChannelFormProps): React.ReactElement {
  ...
  React.useEffect(() => {
    if (isEdit && channel && !apiKeyLoaded) {
      window.electronAPI.decryptApiKey(channel.id).then((key) => {
        ...
        console.error('[模型供应商表单] 解密 API Key 失败:', error)
        setApiKeyLoaded(true)
      })
    }
  ...
  const handleSubmit = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      ...
      console.error('[模型供应商表单] 保存失败:', error)
    } finally {
      setSaving(false)
    }
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <SettingsSection title="基本信息">
        <SettingsCard>
          ...
          <SettingsToggle
            label="启用此模型供应商"
            description="关闭后该模型供应商不会在模型选择中出现"
            checked={enabled}
