...
export const agentStreamingReasoningAtom = atom<string>((get) => {
  const currentId = get(currentAgentSessionIdAtom)
  if (!currentId) return ''
  return get(agentStreamingStatesAtom).get(currentId)?.reasoning ?? ''
})
...
export function applyAgentEvent(
  ...
  switch (event.type) {
    ...
    case 'thinking_delta':
      return { ...prev, reasoning: prev.reasoning + event.text }
